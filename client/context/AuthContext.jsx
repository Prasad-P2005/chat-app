...
export const AuthProvider = ({ children }) => {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (data.success) {
        // we got our authenticated user here
  ...
  const connectSocket = (userData) => {
    ...
    newSocket.connect()
    // we got our socket in the context
    setSocket(newSocket)

    newSocket.on("getOnlineUsers", (userIds) => {
      // we got all the online users here in the context
      setOnlineUsers(userIds)
    })
  }

  // login function to handle user authentication and socket connections
  // here state = "login" and credentials will be the req.body
